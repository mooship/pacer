...
- `config.rs` — `Config { quantum, payday, interval }` (serde). `quantum` is cents, `payday` is a weekday `0=Sun..6=Sat`, `interval` is days between payouts; defaults are R50 / Monday / 7. `sanitized()` clamps to safe ranges. `load()`/`save()` read and write `config.toml` under `dirs::config_dir()/pacer/`, falling back to defaults on any error.
...
- `main.rs` — loads `Config` then `ratatui::init()` / event loop / `ratatui::restore()`. Arrow/Home/End/Delete edit the active field; `F2` opens settings from any screen. Ctrl+C exits from any screen; on Results `q` quits and `s` writes `app.csv()` to `EXPORT_PATH`.
